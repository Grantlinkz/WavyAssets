...
- **`tools/UI/1a global logo/code.html`**: Institutional vector badge SVG geometry with polygon vault crest, WavyAssetsS typography, and green SECURED indicator.
- **`tools/UI/1 WavyAssets_institutional_terminal_multi_asset_wealth_management/code.html`**: Header strip, live syndicate ticker, terminal workspace layouts and color class names.
...
- Update document `<title>` to: `"WavyAssets — Sovereign Multi-Asset Institutional Terminal"`.
