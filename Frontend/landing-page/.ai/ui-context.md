...
- **Dual Typographic Pairing**: Neutral human-engineered grotesque headers (`Inter`) for structural navigation combined with relentless tabular monospaced figures (`JetBrains Mono`) for all financial data.
...
*(Luxury Light Mode shifts canvas to `#F8F9FA`, surfaces to `#FFFFFF`, borders to `#E5E7EB`, and text to `#111827`, with Gold shifting to `#B89324`.)*
...
| :--- | :--- | :--- | :--- | :--- |
| **Headline XL** | `Inter` | 32px / 38px | 600 (Semibold) | `-0.02em` |
| **Headline LG** | `Inter` | 24px / 30px | 600 (Semibold) | `-0.015em` |
| **Headline SM** | `Inter` | 18px / 24px | 500 (Medium) | `-0.01em` |
| **Label Caps (Badges)**| `Inter` | 11px / 14px | 600 (Semibold) | `+0.08em` (Uppercase) |
| **Data Metric LG** | `JetBrains Mono` | 28px / 32px | 600 (Semibold) | `-0.02em` (Tabular) |
| **Data Metric MD** | `JetBrains Mono` | 18px / 24px | 500 (Medium) | `-0.01em` (Tabular) |
| **Body MD (Data)** | `JetBrains Mono` | 13px / 18px | 400 (Regular) | `0em` |
| **Body SM (Data)** | `JetBrains Mono` | 12px / 16px | 400 (Regular) | `0em` |
| **Data Micro** | `JetBrains Mono` | 10px / 12px | 500 (Medium) | `+0.02em` |
...
- Background: `#D4AF37` (Gold), Text: `#08090B` (`Inter` 12px semibold, uppercase).
...
- Positive: Background `rgba(0, 194, 136, 0.10)`, Text `#00C288`, `JetBrains Mono` 11px.
- Negative: Background `rgba(255, 77, 77, 0.10)`, Text `#FF4D4D`, `JetBrains Mono` 11px.
