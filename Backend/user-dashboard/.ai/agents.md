...
4. **Cross-Domain Session & Authentication Handoff**: Safely consume ephemeral single-use handoff tickets issued by `Backend/landing-page` (`POST /api/v1/auth/exchange-ticket`). The producer (`Backend/landing-page`) generates a cryptographically random 32-byte hex token, computes its deterministic HMAC-SHA256 hash (UTF-8 string input, lowercase hex output) using `HANDOFF_TICKET_SECRET`, and stores it in `Session.handoffTicketHash` with a 120-second TTL. The consumer (`Backend/user-dashboard`) verifies the token by recomputing the identical HMAC-SHA256 hash and atomically burns the ticket upon redemption, issuing 15-minute access JWTs and rotating secure HttpOnly refresh tokens.
